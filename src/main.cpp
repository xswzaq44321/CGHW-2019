...
int main(void)
{
    ...
    // Do not remove {}, why???
    {
        ...
        static bool prev_flat_shading = flat_shading;
        bool bling_phong = false;
        bool per_vertex = false;
        static bool prev_per_vertex = per_vertex;
        glm::vec3 light_pos{-2.2, 2.2, 2.2};
        ...
        while (!glfwWindowShouldClose(window))
        {
            ...
            if (prev_per_vertex ^ per_vertex)
            {
                prev_per_vertex = per_vertex;
                if (per_vertex)
                {
                    prog = Program::LoadFromFile(
                        "../resource/vs2.vert",
                        "../resource/gs2.geom",
                        "../resource/fs2.frag");
                }
            ...
            if (prev_flat_shading ^ flat_shading)
            {
                prev_flat_shading = flat_shading;
                if (flat_shading)
                {
                    mesh = StaticMesh::LoadMesh2("../resource/sphere.obj");
                }
                else
                {
                    mesh = StaticMesh::LoadMesh("../resource/sphere.obj");
                }
            }
            ...
            // 2. Show a simple window that we create ourselves. We use a Begin/End pair to created a named window.
            {
                ...
                ImGui::Checkbox("Per-vertex", &per_vertex);
